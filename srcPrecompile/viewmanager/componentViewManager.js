...
const componentViewManager = {
    ...
    getTabState(apogeeView,component,oldTabState,tabStateJson) { 
        ...
        let cellStateManager = apogeeView.getCellStateManager()
        childrenWithCells.forEach(child => {
            let cellState = cellStateManager.getState(child.getId())
            if(cellState) {
                newCellStateArray.push(cellState)
            }
        })
